...
%\cite{ioe}
% Nowadays, some novel concepts are introduced in the area 
% of energy research such as the second generation of smart 
% grids (smart grid 2.0) [15], smart energy, future energy 
% networks [16], industry 4.0 [17], which are connected with 
% internet and telecommunications technologies. 
% Smart grid 2.0 refers to the next generation of smart grids 
% which will be implemented from 2020 which has specific 
% distinctions such as increase in interactions between supplysides and demand-side using advanced smart metering 
% infrastructures, share of energy and its correlated information 
% between deserved players using informatics infrastructure, and 
% the plug & play capability, which means the ability to provide 
% energy even by tiny-scale generation resources (e.g. V2Gs) so 
% that they can be able to deliver power to the grid as easy as 
% plugging in a plug into an outlet. Such a consumer which can 
% sometimes play the role of a provider is called “prosumers”. It 
% is noticeable that the ability of connection or disconnection of 
% distributed generation at each desired moment is not possible 
% at the current time. However, in smart grid 2.0, a wider range 
% of consumer devices and equipment will be in control which 
% 628
% provides more flexibility for power systems. The control of 
% such a massive number of elements cannot be viable without 
% improving the smartness of grids. It is alleged that more than 
% 50% of demand will be controlled by real-time controllers in 
% smart grids 2.0. In such an environment, all players can 
% participate in power markets and monitor its transactions and 
% power exchanges using an internet-based interface in peer-topeer (P2P) mode, in which the buyer and seller are specified. 
% In the first generation of smart grids, the main attention has 
% been focused on the development of fundamental prerequisites 
% such as advanced metering infrastructures (AMI), and the 
% installation of them in the local distribution networks. 
% However, the smart grid concept will be more globalized at a 
% wider level using IoT-based interfaces and will be inclined 
% toward a macro-level and worldwide pervasiveness. The smart 
% grid 2.0 must be able to keep working after any contingency 
% automatically and effectively so that the infected region 
% immediately begin to be supplied using self-healing schemes, 
% which highly depends on real-time data gathering and 
% analysis. In addition, the proper application programming 
% interface (API) will be employed as a platform to display 
% information for P2P and third-party access [18].
...
%  Strengthening the grid—ensuring that there is sufficient transmission capacity to interconnect energy resources, especially renewable resources; 
%  Moving offshore—developing the most efficient connections for offshore wind farms and for other marine 
% technologies; 
%  Developing decentralized architectures—enabling smalller scale electricity supply systems to operate harmoniously with the total system; 
%  Communications—delivering the communications infrastructure to allow potentially millions of parties to 
...
% must accommodate the needs of all consumers, electric vehicles are particularly emphasized due to their 
% mobile and highly dispersed character and possible 
% massive deployment in the next years, what would 
% yield a major challenge for the future electricity networks [2]. 
...
% the lifetime of an appliance. Devices that communicate wirelessly with their facility energymanagement systems must broadcast powerful-enough signals, or other technical barriers to 
...
% generated by DG resources will continue to be a problem until a cost-effective, lowmaintenance solution is introduced. Trends suggest this might be done with highly efficient 
...
% grid during high-price on-peak hours, will not be feasible. The lack of cost-effective, lowmaintenance batteries is a particular hindrance for renewable energies such as solar and wind 
...
% Because of these concerns, PHEVs will be unlikely to penetrate all markets, leaving heavyduty and long-range vehicles, such as semi-trucks, and high-performance vehicles such as 
